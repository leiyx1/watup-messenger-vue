<template>
  <div>
    <el-dialog :visible.sync="dialogVisible" title="移除群员" width="600px" center="true" :modal-append-to-body="false">
      <div id="dialog">
        <div id="search-input">
          <el-input v-model="search" placeholder="搜索群员" size="mini" prefix-icon="el-icon-search"></el-input>
        </div>
        <div id="friend-list">
          <el-table
              :data="members.filter(data => data.username.toLowerCase().includes(search.toLowerCase())&&(data.id!==this.$store.state.user.id))"
              style="width: 100%"
              height="300px"
      ...
      <span slot="footer">
        <el-button @click="handleRemoveFromGroup"> 确认 </el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<script>
    export default {
        name: "RemoveFriendFromGroupDialog",
        props: ['visible','groupId','members'],
        data() {
            return {
        ...
        methods: {
            ...
            handleRemoveFromGroup() {
                ...
                [].forEach.call(this.selectedFriends, selectedFriend => {
                    ...
                    this.$axios
                        .delete("/api/group/"+this.groupId+"?access_token="+this.$store.state.user.access_token+"&userId="+selectedFriend.id)
                        .then((res) => {
                            console.log(res);
                        })
                        .catch(function (error) {
                            console.log(error);
                            failedFriends.push(selectedFriend.id)
                        })
                });
